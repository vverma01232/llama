...
RUN mkdir -p /ollama-bin
RUN curl -fsSL https://ollama.com/install.sh | sh -s -- -b /ollama-bin
...
COPY --from=builder /ollama-bin/.ollama /root/.ollama
COPY --from=builder /ollama-bin/ollama /usr/local/bin/ollama
